...
void	handle_child_process(t_input data, int **pipes, int cmd_index)
{
	if (cmd_index == 0)
		...
		setup_pipe_input(pipes[cmd_index - 1]);
	if (cmd_index == data.num_cmds - 1)
		setup_output_file(data.argv[cmd_index + 3]);
	else
		setup_pipe_output(pipes[cmd_index]);
	close_all_pipes(pipes, data.num_cmds - 1);
	execute_command(data.argv[cmd_index + 2], data.env);
}
